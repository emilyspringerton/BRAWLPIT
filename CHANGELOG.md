...
## 2026-09-13
- S457: fixed the REAL reason the live AI stood still even while training Elo climbed normally ("elos go up and down but when i play them they just stand there") -- `ai_opponent_build_observation()` (packages/common/ai_opponent.h) stopped writing at 21 of the real 31 S430 observation dims (the posture one-hot), even though the 10 relational-feature helpers it needed (`ai_opponent_time_to_any_blast_normalized`, `ai_opponent_facing_toward`, etc.) already existed in this same file, fully correct, just never called. `obs[21..30]` were uninitialized stack garbage fed straight into the live policy net every frame -- training stayed healthy because it's pure Python (`rl_env_packet.py`'s own `build_observation` always filled all 31), so this was invisible in Elo. Added the missing 10-term block using the existing helpers; hand-verified bit-for-bit against `rl_env_packet.py`'s own `build_observation()` for an identical synthetic state (all 31 dims match exactly). New `tests/test_ai_opponent.c` locks this down (asserts the real relational-feature values, not just non-crash), wired into `scripts/build.sh`; full build + all 5 test suites pass.
- S456: fixed gen1 "main"'s zero-variance, no-buttons policy collapsing permanently -- SB3's PPO defaults `ent_coef=0.0`, so nothing in the loss ever fought the Gaussian action distribution's log_std drifting toward zero (and once it's very negative, the recovery gradient itself is nearly flat, so it never comes back). `_fresh_model`/`_load_resumed_model_or_fresh` now take `ent_coef` (default `DEFAULT_ENT_COEF=0.01`, SB3's own commonly-cited continuous-control value), exposed as a new `--ent-coef` flag and wired into all 3 call sites in `main()`. Test suite's own mocked-call assertion updated to match the new kwarg; 34/34 pass.
- S455: fixed a real crash discovered live -- resuming from the >1600 Elo lineage S454 just re-enabled hit `PPO.load`'s `ValueError` (S430's own documented 21->31 OBS_SIZE bump, every registry checkpoint predates it) and took down the whole training run. New `_load_resumed_model_or_fresh()` catches the space-mismatch ValueError specifically, warns loudly, falls back to a fresh model for that role only. 3 new tests, 34/34 total pass. Real, honest gap left open: the whole >1600 Elo lineage is now permanently unusable for direct warm-start under the new obs space -- only a weight-surgery migration (zero-pad the old 21-dim input layer to 31) could actually recover it. Apple #19465. (sess-20260905-0720-ec33e7c5)
- S452: --resume-from-registry now prints a loud WARNING (naming the best disabled checkpoint's id/gen/elo) instead of a silent 'starts fresh' when a role's entire history is disabled rather than nonexistent -- diagnosed a real Elo-spike-then-loss report as 94/role disabled checkpoints (incl. main's ~1700-1900 Elo lineage), not an overwrite bug (sess-20260905-0720-ec33e7c5)
- S451: added a regression guard for MAIN's live PPO model -- if a generation's evaluated Elo drops >=100 below MAIN's own best-ever, the live model reverts to that best checkpoint instead of compounding the regression (new _should_revert_main(), 6 new tests). Diagnosed a real, directly-observed Elo spike-then-collapse (1800 Elo with real movement, then reverted to dormant) as classic PPO catastrophic forgetting in self-play. (sess-20260905-0720-ec33e7c5)
- S449: Hyperbot-style real-time half-life discounting scales the S442 inactivity penalty up as the match clock runs down (new time_pressure_multiplier()); S450: fixed a real --fast-forward busy-spin bug in apps/server/src/main.c (idle CPU ~100% -> ~0%) by blocking on select() instead of a tight non-blocking recvfrom() loop, making the server genuinely event-driven (sess-20260905-0720-ec33e7c5)
...
- feat(rl): S421-02/S421-03 -- native client actually loads+uses the selected AI opponent (real MLP inference in C, cross-language-parity-verified against SB3; LZ4-compressed weights download; always-on 'AI: DEFAULT'/'AI: <name>' HUD), plus --level <name> to train/play on a named registry level (STAGE_CUSTOM_MEMORY, survives PACKET_RESET_MATCH). Both live-verified with real Xvfb+XTest screenshots. Commits b0941a3, a2be366. (sess-20260905-0720-ec33e7c5)
- feat(rl): S419 gap-filling -- real PACKET_RESET_MATCH/ACK (found+fixed 2 more live bugs verifying it: local_init_match only inits slot 0, and PacketClient gave up on stray backlog packets), --port flag, a real 3-tier reward design (edge-danger/recovery/edgeguard-conversion shaping conditioned on the commander posture), scripts/rl_train_packet.py orchestrator (3 archetypes, 1 process, per-generation league registration), and a Colab notebook (notebooks/brawlpit_rl_training.ipynb) to actually run training. 76 Python tests + 2 C binaries pass. Commit 07e060f. (sess-20260905-0720-ec33e7c5)
