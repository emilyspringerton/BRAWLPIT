#!/usr/bin/env python3
"""
scripts/rl_league.py -- AlphaStar-style league training (S419, ported verbatim from
REDGARDEN/scripts/rl_league.py -- founder real-time: "take autocurriculum doctrine (alpha star
league) find the recent additions to the REDGARDEN docs to add that"). Zero REDGARDEN-specific
coupling in this file (pure Python, generic checkpoint paths -- LeagueManager/LeagueRole/PFSP math
never reference any REDGARDEN game type), so it's reused here as-is rather than reimplemented --
matching this monorepo's own established precedent for a related game needing the same RL infra
(ECOWAR's own apps/arena_training/src/headless.c was itself ported from SHANKPIT/REDGARDEN's
identical shape the same way). See REDGARDEN/NORTHSTAR.md §25.4.1 for the full original design
doc and rationale; not re-derived here.
